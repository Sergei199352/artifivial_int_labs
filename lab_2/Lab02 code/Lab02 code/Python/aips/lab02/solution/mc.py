...
class McState(search.State):
    ...
    """Compute a hash value fro them McState object.
...
"""the M&C problem using DFS
as a subclass of the BFS version but overrding the addChild method
"""
class McProblemDFS(McProblem):
    def __init__(self,initialState,goalState):
        super().__init__(initialState,goalState)
        self.goalState=goalState
        
    #override addChild method
    """Override addChild method inherited from superclass to give a DFS behaviour.
    :param fringe: The list of nodes to be explored.
    :type fringe: A list of node objects.
    :param childNode: The new node to be added into the fringe.
    :type childNode: A node.
    """
    def addChild(self, fringe, childNode):
        fringe.insert(0,childNode)      #put new node to beginning of fringe to give DFS
