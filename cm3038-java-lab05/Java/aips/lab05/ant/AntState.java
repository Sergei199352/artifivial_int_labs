...
public class AntState implements State {
	...
	if (this.y < this.world.grid.length -1){
		if (!this.world.grid[y+1][x]){
			...
			AntState nexState = this.applyAction(action);
	...
	if (this.x < this.world.grid[0].length-1){
		if (!this.world.grid[y][x+1]){
...
for (int y=0;y<this.world.grid.length;y++)
	...
	if (this.x==x && this.y==y)		//if x & y equals to x & y of ant so in order to find the ant we need to look at the coordinates of the ant
	//since she is not in the array
